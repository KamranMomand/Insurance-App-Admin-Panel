...
<div class="container">
    <div class="content-wrapper">
        <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="text-center"><span style="color:#FF4A23;">Plans</span> Section</h4>
                        <form class="forms-sample" id="adduser" method="post">
                            <div class="form-group">
                                <label for="group">Select Policy Type</label>
                                <select id="type_select" name="policy_type" class="form-control">
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="group">Select Duration</label>
                                <select id="duration_select" name="duration" class="form-control">
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="group">Monthly Amount</label>
                                <input type="text" name="monthly_amount" class="form-control" id="group"
                                    placeholder="Monthly Amount" required>
                            </div>
                            <div class="form-group">
                                <label for="group">Other Payment</label>
                                <input type="text" name="other_payment" class="form-control" id="group"
                                    placeholder="Other Amount" required>
                            </div>
                            <div class="form-group">
                                <label for="group">Enter Benefits</label>
                                <textarea rows="5" cols="100%" name="benefits" class="form-control" id="group" placeholder="Benefits" required></textarea>
                            </div>
                            <!-- <div class="form-group">
                                <label for="group">Benefits</label>
                                <input type="text" name="benefits" class="form-control" id="group"
            ...
            <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Plans Details</h4>
                        </p>
    ...
    $('#submituser').on('click', function() {
        ...
        // type_select
        var policyID = values[0].value;
        var policyName = $("#type_select").find('option:selected').text();
        var duration = values[1].value;
        var monthly_amount = values[2].value;
        var other_payment = values[3].value;
        var benefits = values[4].value;

        var userID = lastIndex + 1;

        console.log(values);

        firebase.database().ref('Plans/' + userID).set({
            
            policy_type_id: policyID,
    ...
    $('body').on('click', '.updateData', function() {
        ...
        firebase.database().ref('Plans/' + updateID).on('value', function(snapshot) {
    ...
    $('.updateUser').on('click', function() {
        ...
        var updates = {};
        updates['/Plans/' + updateID] = postData;
    ...
    $('.deleteRecord').on('click', function() {
        ...
        firebase.database().ref('Plans/' + id).remove();
